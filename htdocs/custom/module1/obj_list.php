...
require_once __DIR__.'/class/obj.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('module1', 'obj', 'write');
	$permissiontodelete = $user->hasRight('module1', 'obj', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("module1")) {
	accessforbidden('Module module1 not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	$objectclass = 'Obj';
	$objectlabel = 'Obj';
	$uploaddir = $conf->module1->dir_output;
...
$title = $langs->trans("Objs");
//$help_url = "EN:Module_Obj|FR:Module_Obj_FR|ES:Módulo_Obj";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/module1/obj_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-module1 page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/module1/obj_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendObjRef";
$modelmail = "obj";
$objecttmp = new Obj($db);
